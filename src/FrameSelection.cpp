...
std::string FrameSelection::genemarkst(std::map<std::string,QuerySequence> &SEQUENCES) {
    ...
    final_out = (boostFS::path(_frame_outpath) / file_name).string() + ".faa";
    lst_file = file_name.string() + ".lst";
    out_lst = (boostFS::path(_frame_outpath) / boostFS::path(lst_file)).string();
    ...
    if (_overwrite) {
        ...
        if (file_exists(final_out) && file_exists(out_lst)) {
            print_debug("File found at: " + final_out + "\n"
        ...
        print_debug("File not found at " + final_out + " so continuing frame selection");
    }
